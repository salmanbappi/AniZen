...
fun SubtitleDelayPanel(
    ...
    ConstraintLayout(
        ...
        var delay by remember { mutableIntStateOf(((MPVLib.getPropertyDouble("sub-delay") ?: 0.0) * 1000).roundToInt()) }
        var secondaryDelay by remember {
            mutableIntStateOf(((MPVLib.getPropertyDouble("secondary-sub-delay") ?: 0.0) * 1000).roundToInt())
        }
        var speed by remember { mutableFloatStateOf((MPVLib.getPropertyDouble("sub-speed") ?: 1.0).toFloat()) }
        ...
        LaunchedEffect(affectedSubtitle) {
            ...
            delay = ((MPVLib.getPropertyDouble("sub-delay") ?: 0.0) * 1000).toInt()
        }
        SubtitleDelayCard(
